...
final class PayloadLivenessBenchmarks: XCTestCase {
    ...
    private struct BenchmarkConfig {
        ...
        static func current() -> BenchmarkConfig {
            ...
            let defaultOutputPath = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
                ...
                .appendingPathComponent("payload-liveness-benchmark.json")
                .path

            return BenchmarkConfig(
                scale: scale,
                pairCount: env["WAX_PAYLOAD_LIVENESS_PAIRS"].flatMap(Int.init)
                    ?? (scale == .smoke ? 3_000 : 24_000),
                deletedFrameCount: env["WAX_PAYLOAD_LIVENESS_DELETED"].flatMap(Int.init)
                    ?? (scale == .smoke ? 500 : 4_000),
                batchSize: env["WAX_PAYLOAD_LIVENESS_BATCH_SIZE"].flatMap(Int.init)
                    ?? (scale == .smoke ? 512 : 2_048),
                iterations: env["WAX_PAYLOAD_LIVENESS_ITERATIONS"].flatMap(Int.init)
                    ?? (scale == .smoke ? 9 : 21),
                warmupIterations: env["WAX_PAYLOAD_LIVENESS_WARMUP"].flatMap(Int.init) ?? 3,
                outputPath: env["WAX_BENCHMARK_PAYLOAD_LIVENESS_OUTPUT"] ?? defaultOutputPath
            )
        }
    }
    ...
    func testCommittedPayloadLivenessMetrics() async throws {
        guard ProcessInfo.processInfo.environment["WAX_BENCHMARK_PAYLOAD_LIVENESS"] == "1" else {
            throw XCTSkip("Set WAX_BENCHMARK_PAYLOAD_LIVENESS=1 to run payload liveness benchmarks.")
        }
        ...
        try await TempFiles.withTempFile { url in
            ...
            let legacyBytes = await legacyPayloadLivenessBytes(wax: wax)
            let optimizedBytes = await wax.committedPayloadLivenessBytes()
            XCTAssertEqual(legacyBytes.totalPayloadBytes, optimizedBytes.totalPayloadBytes)
            XCTAssertEqual(legacyBytes.deadPayloadBytes, optimizedBytes.deadPayloadBytes)

            for _ in 0..<config.warmupIterations {
                _ = await legacyPayloadLivenessBytes(wax: wax)
                _ = await wax.committedPayloadLivenessBytes()
            }

            let legacy = try await measureMode(iterations: config.iterations) {
                _ = await self.legacyPayloadLivenessBytes(wax: wax)
            }
            ...
            XCTAssertLessThanOrEqual(
                ...
                "Optimized payload liveness p95 regressed materially versus the legacy frameMetas reducer."
            )
            XCTAssertLessThanOrEqual(
                optimized.latencyMs.p99Ms,
                legacy.latencyMs.p99Ms * 1.10 + 0.5,
                "Optimized payload liveness p99 regressed materially versus the legacy frameMetas reducer."
            )
            ...
            let report = BenchmarkReport(
                ...
                deletedFrameCount: config.deletedFrameCount,
                legacy: legacy,
                optimized: optimized,
    ...
    private func seedStore(at url: URL, config: BenchmarkConfig) async throws {
        ...
        let deadPayload = Data(repeating: 0x41, count: 512)

        for batchStart in stride(from: 0, to: config.pairCount, by: batchSize) {
            let batchEnd = min(batchStart + batchSize, config.pairCount)
            for index in batchStart..<batchEnd {
                let oldFrame = try await wax.put(
                    deadPayload,
                    options: FrameMetaSubset(searchText: "superseded frame \(index)")
                )
                let replacement = try await wax.put(
                    Data("replacement \(index)".utf8),
                    options: FrameMetaSubset(searchText: "replacement \(index)")
                )
                try await wax.supersede(supersededId: oldFrame, supersedingId: replacement)
            }
            try await wax.commit()
        }

        for batchStart in stride(from: 0, to: config.deletedFrameCount, by: batchSize) {
            let batchEnd = min(batchStart + batchSize, config.deletedFrameCount)
            for index in batchStart..<batchEnd {
                let frameID = try await wax.put(
                    deadPayload,
                    options: FrameMetaSubset(searchText: "deleted frame \(index)")
                )
    ...
    private func legacyPayloadLivenessBytes(
        wax: Wax
    ) async -> (totalPayloadBytes: UInt64, deadPayloadBytes: UInt64) {
