...
<p align="center">
  ...
  オンデバイス、プライベート、ポータブル — サーバー不要、クラウド不要、インフラ構成不要。
</p>
...
これを `main.swift` ファイルにコピー＆ペーストして、すぐに使い始めることができます。
...
struct AgentMemory {
    static func main() async throws {
        ...
        // 1. メモリストアを初期化 (オンデバイスのMiniLMエンベディング)
        let memory = try await MemoryOrchestrator.openMiniLM(at: url)

        // 2. 新しいメモリを保存
        try await memory.remember("ユーザーはSwiftUIで習慣トラッカーを構築しています。")

        // 3. セマンティック検索を実行
        let context = try await memory.recall(query: "ユーザーは何を構築していますか？")

        if let bestMatch = context.items.first {
            print("想起: \(bestMatch.text)") 
            // 出力: "想起: ユーザーはSwiftUIで習慣トラッカーを構築しています。"
        }
    }
}
```
